-- CREATE OR REPLACE TABLE `sas-saude-alto-custo-hml.SBX_ALTO_CUSTO.TB_EXP_SDCC405_ONCOLOGIA_VPP` AS
...
DAT_INICIO_PROGRAMA_SAUDE_ATIVA_1,
DSC_PROGRAMA_SAUDE_ATIVA_1,
QTD_SCORE_CARDIOPATIA SCORE_CARDIO,
QTD_SCORE_DIABETES SCORE_DIABETES
...
FROM `self-service-saude.VW_CONS_SAU.TB_DIM_BENEFICIARIO` 
)
...
LEFT JOIN DIM_BENEF DB
ON A.COD_BENEFICIARIO_CHAVE = DB.COD_BENEFICIARIO_CHAVE
...
-- FILTRO VW
-- WHERE DAT_INCLUSAO = DATE_SUB(CURRENT_DATE, INTERVAL 1 DAY)


-- NUM_IDADE_BENEFICIARIO TABELA VPP
-- COD_SEXO_BENEFICIARIO TABELA VPP
-- NME_PLANO TABELA VPP
-- DSC_CARTEIRA_EMPRESA TABELA VPPP
-- NME_FANTASIA_PRESTADOR TABELA VPP
-- QTD_DIAS_INTERNADO TABELA SINISTRO 
-- QTD_CONSULTA TABELA SINISTRO
-- QTD_BIOPSIAS TABELA SINISTRO 
-- QTD_PET TABELA SINISTRO
-- QTD_QUIMIO TABELA SINISTRO
-- QTD_RADIO TABELA SINISTRO
-- DSC_SUBGRUPO_SERVICO TEM NA DE SINISTRO, MAS POSSO TRAZER NA DE VPP
-- DSC_SERVICO TEM ANAS DUAS
-- DAT_EXECUCAO_SINISTRO TABELA SINISTRO
-- DSC_ECOG TABELA VPP
-- DSC_FINALIDAD_TRAT TABLEA VPP
-- DSC_ESTAD_TUMOR VPP
-- DSC_CIRURGIA VPP
-- DSC_QUIMIOTERAPIA VPP